...
class ChatService {
  ...
  async loadTranscriptionData(resultFilePath) {
    try {
      ...
      const jobId = data.job_id || path.basename(resultFilePath, '_result.json');
  ...
  async processQuery(query, sessionId = 'default', fileId = null) {
    try {
      ...
      if (!openaiClient) {
        ...
        const result = await this.generateAIResponse(query, transcriptionContext, session);
    ...
    try {
      ...
      if (transcriptionContext) {
        ...
        const transcriptText = transcriptionContext.transcript
          .map(seg => `${seg.speaker_name} (${seg.start.toFixed(1)}s): ${seg.text}`)
          .join('\n');
...
Please provide a helpful and accurate answer based on the meeting transcript and context. If the information isn't available in the transcript, say so clearly.`;
      ...
      const response = await openaiClient.chat.completions.create({
        ...
        messages: [
          {
            role: "system",
            content: "You are a helpful meeting assistant. Provide accurate answers based on the transcript data. If information isn't available, be honest about limitations. Keep responses concise but informative."
          },
          {
            role: "user",
            content: prompt
  ...
  generateRuleBasedResponse(query, transcriptionContext) {
    ...
    if (lowerQuery.includes('summary') || lowerQuery.includes('summarize')) {
      return transcriptionContext.summary || "No summary is available for this meeting.";
    }
    
    // Speaker questions
    if (lowerQuery.includes('speaker') || lowerQuery.includes('who') || lowerQuery.includes('participant')) {
    ...
    if (lowerQuery.includes('action') || lowerQuery.includes('task') || lowerQuery.includes('todo')) {
      ...
      return `Here are the action items from the meeting:\n${actionItems.map((item, index) => 
        `${index + 1}. ${item.task} (Assigned to: ${item.assignee}, Deadline: ${item.deadline})`
      ).join('\n')}`;
    }
    
    // Key decisions
    if (lowerQuery.includes('decision') || lowerQuery.includes('decide') || lowerQuery.includes('conclusion')) {
    ...
    // Transcript search
    if (lowerQuery.includes('said') || lowerQuery.includes('mention') || lowerQuery.includes('discuss')) {
      const transcript = transcriptionContext.transcript || [];
      const relevantSegments = transcript.filter(seg => 
        seg.text.toLowerCase().includes(lowerQuery.replace(/.*(?:said|mention|discuss)\s*/i, ''))
      );
      
      if (relevantSegments.length > 0) {
        return `Here's what was mentioned:\n${relevantSegments.slice(0, 3).map(seg => 
    ...
    // Default response
    return `I found a meeting transcript with ${transcriptionContext.transcript?.length || 0} segments from ${transcriptionContext.speakers?.length || 0} speakers. You can ask me about the summary, speakers, action items, key decisions, or search for specific topics mentioned in the conversation.`;
  }
